...
namespace LBM
{
    public class DataHelper
    {
        ...
        public static List<EF.Marge> GetMarges()
        {
            List<EF.Marge> marges;
            using (var ctx = new DatabaseContext())
            {
                marges = ctx.Marges.ToList();
            }

            return marges;
        }

        public static EF.Marge GetMarge(string key, string gewasnaam)
        {
            EF.Marge marge;
            using (var ctx = new DatabaseContext())
            {
                marge = ctx.Marges.Where(x => x.Key == key && x.GewasNaam == gewasnaam).FirstOrDefault();
            }
